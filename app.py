...
class Appointment(db.Model):
    ...
    is_emergency = db.Column(db.Integer, default=0)
...
with app.app_context():
    ...
    admin_exists = User.query.filter_by(username="admin", role="admin").first()
    if not admin_exists:
...
def add_doctor():
    ...
    try:
        ...
        flash(f"Doctor added! Login username: {username}", "success")
        return redirect("/doctors")
...
def update_doctor(id):
    ...
    user = db_fetchone(
        "SELECT username FROM users WHERE ref_id=:id AND role='doctor'",
        {"id": id}
    )
